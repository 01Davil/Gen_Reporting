...
public partial class Hr_GenerateSalary : System.Web.UI.Page
{
    ...
    public static List<Name> GetDateSalary()
    {
        ...
        try
        {
            ...
            cmd = new SqlCommand("AccountGetDate", con);
        ...
        foreach (DataRow dtt in dt.Rows)
        {
            Data.Add(new Name
            {
                Year = dtt["id"].ToString(),
                Month = dtt["mname"].ToString(),
            });
        }
    ...
    public static string ApprovedSalary(string YNo, string MNo)
    {
        int Y = int.Parse(YNo);
        int M = int.Parse(MNo);
        ...
        try
        {
            ...
            SqlConnection con = new SqlConnection(conn);
            con.Open();
            cmd = new SqlCommand("HR_Approved_SalaryDate", con);
            cmd.Parameters.AddWithValue("@Year", YNo);
            cmd.Parameters.AddWithValue("@Monthno", MNo);
            cmd.Parameters.AddWithValue("@TotalDay", Day);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.ExecuteNonQuery();
            con.Close();
            return "Done";
