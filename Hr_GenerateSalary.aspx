<%@ Page Title="" Language="C#" MasterPageFile="~/HRMasterPage.master" AutoEventWireup="true" CodeFile="Hr_GenerateSalary.aspx.cs" Inherits="Hr_GenerateSalary" %>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#ApprovedSalary").click(function (e) {
            e.preventDefault();
            $("#EmployeeSalaryModal2").modal('show');
        });

        $("#Conbtn").click(function (e) {
            e.preventDefault();
            var Y = $("#YearDrop option:selected").text();
            var M = $("#MonthDrop option:selected").val();
            if (Y == "Select Year" || M == "undefined" || M == undefined || M == "Select Month" || M == 0 || M== '0') {
                Swal.fire({
                    icon: 'info',
                    title: 'info',
                    width: '300px',
                ...
                $.ajax({
                    type: "POST",
                    url: "Hr_GenerateSalary.aspx/ApprovedSalary",
                    data: JSON.stringify(objaa),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (response) {
                        if (response.d == "Done") {
                            // toastr.success('Salary Generate', 'Success');
                            Swal.fire({
                                icon: 'success',
                                title: 'success',
                                width: '300px',
                                text: ' Salary Generate .!'
                            })
                            setTimeout(function () {
                                location.reload();
                            }, 1000);
    ...
    function GetSalary() {
        ...
        $.ajax({
            type: "POST",
            url: "Hr_GenerateSalary.aspx/ShowSalaryhistory",
            data: "{}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                ...
                for (var i = 0; i <= xmlDoc.length - 1; i++) {
                    jQuery('<tr> <td>' + j + '</td> <td>' + xmlDoc[i].EmpCode + '</td> <td>' + xmlDoc[i].EmpName + '</td> '
                       ...
                       + '  <td><button type="button" class="btn btn-warning form-control"  id="update"' + j + ' onclick="EditBg(' + j + ')" > <i class="bi bi-pencil-square"></i>Edit </button></td>      </tr>').appendTo("#BanlSlipTable");
