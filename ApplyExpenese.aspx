<%@ Page Title="" Language="C#" MasterPageFile="~/MasterGen.master" AutoEventWireup="true" CodeFile="ApplyExpenese.aspx.cs" Inherits="ApplyExpenese" %>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(function () {
                  var formData = new FormData();
                        ...
                        $.ajax({
                            url: 'WebServerFile/WebService.asmx/TypeofExpenese',
                            type: 'POST',
                            data: formData,
                            cache: false,
                            contentType: false,
            ...
            $("#BtnSave").click(function (e) {
                ...
                if (Amount.length == 0 || Amount == ' ' || Purpose.length == 0 || Purpose == '' || TypeExp.length == 0 || TypeExp == " ") {
                    ...
                    if (file == 0) {
                        ...
                        $.ajax({
                            url: 'WebServerFile/WebService.asmx/SaveExpenese',
                            type: 'POST',
                            data: formData,
                            cache: false,
                            contentType: false,
                    ...
                    if (Amount != '' || Purpose != '') {
                        ...
                        if ($('#SlipFile')[0].files.length == 0) {
                            ...
                            formData.append("TypeExp", TypeExp);
                            $.ajax({
                                url: 'WebServerFile/WebService.asmx/UpdateExpeneseSP',
                                type: 'POST',
                                data: formData,
                                cache: false,
                                contentType: false,
                            ...
                            formData.append("TypeExp", TypeExp);
                            $.ajax({
                                url: 'WebServerFile/WebService.asmx/UpdateExpenese',
                                type: 'POST',
                                data: formData,
                                cache: false,
                                contentType: false,
        ...
        function GetApplyExpenese(Type) {
            ...
            formData.append("Sno", "<%= Session["LoginSno"].ToString()%>");
             ...
             $.ajax({
                 url: 'WebServerFile/WebService.asmx/GetApplyExpeneseSP',
                 type: 'POST',
                 data: formData,
                 cache: false,
                 contentType: false,
                     ...
                     if (data[0].response == "Fail") {
                         ...
                         for (var i = 0; i < data.length; i++) {
                             ...
                             if (data[i].ApprovedStatus == 'Processing') {
                                 ...
                                 jQuery('<tr><td style=" display :none;"> ' + data[i].URL + '</td> <td style=" display :none;"> ' + data[i].id + '</td> <td>' + j + '</td> <td>' + data[i].TypeExp + '</td> <td>' + data[i].Amount + '</td><td>' + data[i].Purpose + '</td> <td>' + data[i].ApprovedStatus + '</td> <td>' + data[i].ApprovedName + '</td> <td>' + data[i].PaymentStatus + '</td>'
        ...
        function EditFun( j) {
            ...
            var TypeExp = document.getElementById("datatables-dashboard-projects").rows[j].cells.item(3).innerText;
            var Amount = document.getElementById("datatables-dashboard-projects").rows[j].cells.item(4).innerText;
            var Purpose = document.getElementById("datatables-dashboard-projects").rows[j].cells.item(5).innerText;
           $("#Amount").val(Amount);
           $("#Purpose").val(Purpose);
           $("#BtnSave").text("Update");
           $("#Mid").val(id);
           var s = " <option  selected title='already selected'  class='text-danger'>" + TypeExp + "</option>";
           $("#Exp_Type").append(s);
        }
        function DeleteFun(j) {
           // formData
                      var formData = new FormData();
                        ...
                        $.ajax({
                            url: 'WebServerFile/WebService.asmx/DeleteExpenese',
                            type: 'POST',
                            data: formData,
                            cache: false,
                            contentType: false,
    ...
    <div class="card flex-fill">
        ...
        <table id="datatables-dashboard-projects" class="table table-striped my-0">
            <thead>
                <tr>
                    <th>S.No.</th>
                    <th class="d-none d-xl-table-cell">Expenese Type</th>
