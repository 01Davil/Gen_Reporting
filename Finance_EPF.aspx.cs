...
public partial class Finance_EPF : System.Web.UI.Page
{
    ...
    [WebMethod]
    public static List<classYear> GetYear()
    {
        DataTable dt = new DataTable();
        DataSet ds = new DataSet();
        try
        ...
        List<classYear> Data = new List<classYear>();

        foreach (DataRow dtt in dt.Rows)
        {
            Data.Add(new classYear
            {
                YearNo = dtt["id"].ToString(),
                YearName = dtt["YearNo"].ToString(),
            });
        }
